...
def findCsvFiles(goodFolderPath,badFolderPath):
    ...
    for i,file in enumerate(os.listdir(directoryPath)):
        if file.endswith(""):
            folder_name=str(os.path.abspath(directoryPath)+("\\")+str(os.path.basename(file)))
    ...
    for i,file in enumerate(os.listdir(directoryPath)):
        ...
        if file.endswith(""):
            folder_name=str(os.path.abspath(directoryPath)+("\\")+str(os.path.basename(file)))
            print("pahakansio:",folder_name )
            list_bad_foldernames.append(folder_name)
            #print(folder_name, ' folder added')


    '''Search for .CSV files from the sub folders and make list of their paths'''
